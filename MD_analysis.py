...
def create_conditions_file(run_dir: Path, output_dir: Path, run_name: str,
    ...
    forcefields = re.findall(r"ForceField\((.*?)\)", python_text)

    platform = re.search(r"Platform\.getPlatformByName\('([^']+)'\)", python_text,)

    temperature = re.search(r"LangevinIntegrator\((.*?)\*kelvin", python_text,)
    friction = re.search(r"kelvin,\s*(.*?)\/picosecond",python_text,)

    timestep = re.search(r"([0-9.]+)\*picoseconds\)", python_text,)
    md_steps = re.search(r"simulation\.step\((\d+)\)", python_text,)
    ...
    if md_steps and timestep:
        total_ps = (int(md_steps.group(1)) * float(timestep.group(1)))
    ...
    try:
        ...
        water_residues = len(u.select_atoms("resname HOH").residues)
    except Exception:
        pass
...
Force field(s): {forcefields[0] if forcefields else "Unknown"}
Water model: {water_model.group(1) if water_model else "Unknown"}
Water padding (nm): {padding.group(1) if padding else "Unknown"}
Nonbonded method: {nonbonded_method.group(1) if nonbonded_method else "Unknown"}
Nonbonded cutoff (nm): {cutoff.group(1) if cutoff else "Unknown"}
Constraints: {constraints.group(1) if constraints else "Unknown"}

Simulation
----------
Temperature (K): {temperature.group(1) if temperature else "Unknown"}
Friction (1/ps): {friction.group(1) if friction else "Unknown"}
Timestep (ps): {timestep.group(1) if timestep else "Unknown"}
MD steps: {md_steps.group(1) if md_steps else "Unknown"}
Simulation time (ns): {simulation_time_ns}

Reporting
---------
Frames saved: {n_frames}
log.txt interval (steps): {log_interval.group(1) if log_interval else "Unknown"}
trajectory.dcd interval (steps): {dcd_interval.group(1) if dcd_interval else "Unknown"}
